...
uint16_t CRC16_Get8(uint8_t* pu8Data, uint32_t u32Len)
{
    ...
    M0P_CRC->CRC_CR_f.MODE = 0;
    M0P_CRC->CRC_RESULT = 0xFFFF;
    for(u32Index = 0;u32Index<u32Len;u32Index++)
    {
            *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA)))  = pu8Data[u32Index];
    }

    return (M0P_CRC->CRC_RESULT_f.RESULT);
}
...
uint16_t CRC16_Get16(uint16_t* pu16Data, uint32_t u32Len)
{
    ...
    M0P_CRC->CRC_CR_f.MODE = 0;
    M0P_CRC->CRC_RESULT_f.RESULT = 0xFFFF;
    for (u32Index=0; u32Index<u32Len; u32Index++)
    {
        *((volatile uint16_t*)(&(M0P_CRC->CRC_DATA))) = pu16Data[u32Index];
    }
...
uint16_t CRC16_Get32(uint32_t* pu32Data, uint32_t u32Len)
{
    ...
    for (u32Index=0; u32Index<u32Len; u32Index++)
    {
        M0P_CRC->CRC_DATA_f.DATA = pu32Data[u32Index];
    }
...
en_result_t CRC16_Check8(uint8_t* pu8Data, uint32_t u32Len, uint16_t u16CRC)
{
     ...
     *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA))) = (uint8_t)((((uint32_t)u16CRC)>>0)&0xFF);
     *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA))) = (uint8_t)(((uint32_t)u16CRC>>8)&0xFF);
    
    enResult = M0P_CRC->CRC_CR_f.FLAG ? Ok : Error;
...
en_result_t CRC16_Check16(uint16_t* pu16Data, uint32_t u32Len, uint16_t u16CRC)
{
     ...
     *((volatile uint16_t*)(&(M0P_CRC->CRC_DATA))) = u16CRC;
...
en_result_t CRC16_Check32(uint32_t* pu32Data, uint32_t u32Len, uint16_t u16CRC)
{
    ...
    M0P_CRC->CRC_CR_f.MODE = 0;	
    M0P_CRC->CRC_RESULT_f.RESULT = 0xFFFFFFFFu;
    for (u32Index=0; u32Index<u32Len; u32Index++)
    {
        *((volatile uint32_t*)(&(M0P_CRC->CRC_DATA))) = pu32Data[u32Index];
    }
    
     *((volatile uint16_t*)(&(M0P_CRC->CRC_DATA))) = ((uint16_t)u16CRC);
...
uint32_t CRC32_Get8(uint8_t* pu8Data, uint32_t u32Len)
{
    ...
    M0P_CRC->CRC_CR_f.MODE = 1;
    M0P_CRC->CRC_RESULT = 0xFFFFFFFFu;
    for(u32Index = 0;u32Index<u32Len;u32Index++)
    {
            *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA)))  = pu8Data[u32Index];
    }
...
en_result_t CRC32_Check8(uint8_t* pu8Data, uint32_t u32Len, uint32_t u32CRC)
{
     ...
     *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA))) = (uint8_t)((u32CRC>>0)&0xFF);
     *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA))) = (uint8_t)((u32CRC>>8)&0xFF);
     *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA))) = (uint8_t)((u32CRC>>16)&0xFF);
     *((volatile uint8_t*)(&(M0P_CRC->CRC_DATA))) = (uint8_t)((u32CRC>>24)&0xFF);
...
en_result_t CRC32_Check16(uint16_t* pu16Data, uint32_t u32Len, uint32_t u32CRC)
{
     ...
     *((volatile uint16_t*)(&(M0P_CRC->CRC_DATA))) = (uint16_t)((u32CRC>>0)&0xFFFF);
     *((volatile uint16_t*)(&(M0P_CRC->CRC_DATA))) = (uint16_t)((u32CRC>>16)&0xFFFF);
...
en_result_t CRC32_Check32(uint32_t* pu32Data, uint32_t u32Len, uint32_t u32CRC)
{
     ...
     *((volatile uint32_t*)(&(M0P_CRC->CRC_DATA))) = u32CRC;
