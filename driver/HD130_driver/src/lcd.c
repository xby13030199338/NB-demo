 ...
 **
 ******************************************************************************/
 en_result_t LCD_GetSegCom(stc_lcd_segcompara_t stcSegComPara,stc_lcd_segcom_t *pstcSegCom)
{
    en_result_t enRet = Error;
    if(stcSegComPara.u8MaxSeg>40)
    {
        return ErrorInvalidParameter;
    }
    switch(stcSegComPara.enBiasSrc)//seg32_35
    {
        case LcdInRes_High:
        case LcdInRes_Low:
        case LcdInRes_Mid:
    ...
    switch(stcSegComPara.enRunMode)//COM0_7
    {
        case Lcd_StaticMode:
 ...
 **
 ******************************************************************************/
 en_result_t LCD_SetSegCom(stc_lcd_segcom_t stcSegCom)
 {
    en_result_t enRet = Error;
    M0P_LCD->POEN0 = stcSegCom.u32Seg0_31;
    M0P_LCD->POEN1_f.MUX = stcSegCom.bMux;
    M0P_LCD->POEN1_f.C0= stcSegCom.u8Com0_3&0x01;
    M0P_LCD->POEN1_f.C1= stcSegCom.u8Com0_3&0x02;
    M0P_LCD->POEN1_f.C2= stcSegCom.u8Com0_3&0x04;
    M0P_LCD->POEN1_f.C3= stcSegCom.u8Com0_3&0x08;
    M0P_LCD->POEN1 |= (uint32_t)(stcSegCom.Seg32_39VLcdCom7_4_t.SegVLcdCom);
    enRet = Ok;
    return enRet;
 }
  /**
 ...
 **
 ******************************************************************************/
 en_result_t LCD_Init(stc_lcd_config_t stcLcdCfg)
 {
     en_result_t enRet = Error;
     enRet = LCD_SelBiasSrc(stcLcdCfg.enBiasSrc);
     enRet = LCD_SetDuty(stcLcdCfg.enDuty);
     enRet = LCD_SetBias(stcLcdCfg.enBias);
     enRet = LCD_SelCpClk(stcLcdCfg.enCpClk);
     enRet = LCD_SelScanClk(stcLcdCfg.enScanClk);
     enRet = LCD_SetDispMode(stcLcdCfg.enDispMode);
     enRet = LCD_SetClkSrc(stcLcdCfg.enClk);
     if(Ok!=enRet)
     {
        return ErrorInvalidParameter;
     }
     if(stcLcdCfg.bTouchNvic)
     {
         M0P_LCD->CR1_f.IE = 1;
