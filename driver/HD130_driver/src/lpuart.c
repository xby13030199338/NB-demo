 ...
 ** \retval SCNT计算值
 ******************************************************************************/
uint16_t LPUart_CalScnt(uint8_t u8Idx,stc_lpuart_baud_t stcBaud)
{
    stc_lpuart_instance_data_t *pstcData = NULL;
	uint16_t u16Scnt = 0;
    uint8_t u8Over = 0;
    ...
    if(LPUartMode0 == stcBaud.enRunMode)
    {
        return 0;//test
    }
    if((LPUartMode1 == stcBaud.enRunMode)||(LPUartMode3 == stcBaud.enRunMode))
    {
        u8Over = 1<<(4-u8Over);            
        u16Scnt = stcBaud.u32Sclk/(stcBaud.u16Baud*u8Over);
    }
    else
    {
        u8Over = 1<<(5-u8Over);
        u16Scnt = stcBaud.u32Sclk/u8Over; 
    }   
    return u16Scnt;
}
/**
