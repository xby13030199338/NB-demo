...
en_result_t PCNT_Init(stc_pcnt_config_t*  pstcPcntConfig)
{
    ...
    {
        M0P_PCNT->IER = pstcPcntConfig->u8IrqStatus;
...
{
   return  (en_pcnt_direcsel_t)M0P_PCNT->SR0_f.DIR;
}
...
void PCNT_SetIrqStatus(en_pcnt_status_t enStatus)
{
    ...
    {
        case PCNT_S1E:
            M0P_PCNT->IER_f.S1E = 1;
            break;                
        case PCNT_S0E:            
            M0P_PCNT->IER_f.S0E = 1;
            break;                
        case PCNT_BB:            
             M0P_PCNT->IER_f.BB = 1;
            break;                
