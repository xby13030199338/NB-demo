...
en_result_t Spi_ClearStatus(en_spi_channel_t enCh)
{
    ...
    if(Spi0 == enCh)
    {
        M0P_SPI0->ICR_f.INTCLR = 0;
    }
    else
    {
        M0P_SPI1->ICR_f.INTCLR = 0;
    }
    enRet = Ok;
    return enRet;
}
...
en_result_t Spi_FuncEn(en_spi_channel_t enCh,en_spi_func_t enFunc,boolean_t bFlag)
{
    ...
    if(Spi0 == enCh)
    {
        ...
        {
            case SpiRxNeIe:
                M0P_SPI0->CR1_f.RXNEIE = bFlag;
                break;
            case SpiTxEIe:
                M0P_SPI0->CR1_f.TXEIE = bFlag;
                break;
            case SpiDmaTxEn:
                M0P_SPI0->CR1_f.DMATXEN = bFlag;
                break;
            case SpiDmaRxEn:
                M0P_SPI0->CR1_f.DMARXEN = bFlag;
                break;
            default:
    ...
    else
    {
        ...
        {
            case SpiRxNeIe:
                M0P_SPI1->CR1_f.RXNEIE = bFlag;
                break;
            case SpiTxEIe:
                M0P_SPI1->CR1_f.TXEIE = bFlag;
                break;
            case SpiDmaTxEn:
                M0P_SPI1->CR1_f.DMATXEN = bFlag;
                break;
            case SpiDmaRxEn:
                M0P_SPI1->CR1_f.DMARXEN = bFlag;
                break;
            default:
                return ErrorInvalidParameter;
        }  
...
en_result_t Spi_Init(en_spi_channel_t enCh,stc_spi_config_t *pstcSpiConfig)
{
    ...
    if(Spi0 == enCh)
    {
        ...
        M0P_SPI0->CR0_f.MSTR = pstcSpiConfig->bMasterMode;
        M0P_SPI0->CR0_f.CPOL = pstcSpiConfig->bCPOL;
        M0P_SPI0->CR0_f.CPHA = pstcSpiConfig->bCPHA;
        if(pstcSpiConfig->u8BaudRate > SpiClkDiv16)
        {
            M0P_SPI0->CR0_f.SPR2 = 1;      
        }
        M0P_SPI0->CR0 |= (pstcSpiConfig->u8BaudRate&0x03u);    

        M0P_SPI0->STAT = 0x00;

        M0P_SPI0->CR0_f.SPEN = TRUE;
    ...
    else
    {
        ...
        M0P_SPI1->CR0_f.MSTR = pstcSpiConfig->bMasterMode;
        M0P_SPI1->CR0_f.CPOL = pstcSpiConfig->bCPOL;
        M0P_SPI1->CR0_f.CPHA = pstcSpiConfig->bCPHA;
        if(pstcSpiConfig->u8BaudRate > SpiClkDiv16)
        {
            M0P_SPI1->CR0_f.SPR2 = 1;      
        }
        M0P_SPI1->CR0 |= (pstcSpiConfig->u8BaudRate&0x03u);    

        M0P_SPI1->STAT = 0x00;

        M0P_SPI1->CR0_f.SPEN = TRUE;
        if (TRUE == pstcSpiConfig->bIrqEn)
        {
            M0P_SPI1->CR1_f.IE = 1;
