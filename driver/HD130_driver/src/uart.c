 ...
 ******************************************************************************/
uint16_t Uart_CalScnt(uint8_t u8Idx,stc_uart_baud_t stcBaud)
{
    stc_uart_instance_data_t *pstcData = NULL;
	uint16_t u16Scnt = 0;
    uint8_t u8Over = 0;
    ...
    if(UartMode0 == stcBaud.enRunMode)
    {
        return 0;//test
    }
    if((UartMode1 == stcBaud.enRunMode)||(UartMode3 == stcBaud.enRunMode))
    {
        if(0 == u8Over)
        {
            u8Over = 16;
        ...
        u16Scnt = stcBaud.u32Pclk/(stcBaud.u16Baud*u8Over);
    }
    else
    {
        if(0 == u8Over)
       ...
       u16Scnt = stcBaud.u32Pclk/u8Over; 
    }   
    return u16Scnt;
}
/**
