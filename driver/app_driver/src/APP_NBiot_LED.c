...
void NB_iot_SendAT(char *_Cmd)
{
  ...
  Padding_Uart_Tx_Buf((uint8_t *)_Cmd, strlen(_Cmd)); 
...
uint8_t NB_Iot_Get_Nband(void)
{
  ...
  {			
    if(R_State(&NB_Iot_State,RcvOK) == true)
    {
      ...
      {
        addr = strstr(ucRxBuf,"+NBAND:");
        
        if(addr != NULL)
        {
            nband = Ascii_To_Hex( *(addr+ 7));//变量 = (* 指针) 指针 = &变量
            break;
        }
      }
    }
...
uint8_t NB_Iot_Set_Nband(uint8_t nband)
{
  if(nband == 0)
  {
    NB_iot_SendAT("AT+NBAND=5");//查询模组频段
  }
  else if(nband == 1)
  {
    NB_iot_SendAT("AT+NBAND=5");//查询模组频段
  ...
  return(NB_iot_WaitResponse("OK",300/10));
}
